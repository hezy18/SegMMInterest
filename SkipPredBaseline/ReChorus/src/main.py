...
def parse_global_args(parser):
	...
	parser.add_argument('--all_inference', type=str, default='',
						help='Dataset path for inference and save all the scores')
	return parser
...
def main():
	...
	logging.info('Test Before Training: ' + runner.print_res(data_dict['test'])) # TODO:remove/add
	if args.load:
		model.load_model()
	if args.train:
	...
	if len(args.all_inference):
		save_inference_scores(args, runner, reader_name, model_name, model)
	else:
		...
		if args.save_final_results:  # save the prediction results
...
def save_inference_scores(args, runner, reader_name, model_name, model):
	args.dataset = args.all_inference
	corpus_path = os.path.join(args.path, args.dataset, model_name.reader+args.data_appendix+ '.pkl')
	# if not args.regenerate and os.path.exists(corpus_path):
	# 	logging.info('Load corpus from {}'.format(corpus_path))
	# 	corpus = pickle.load(open(corpus_path, 'rb'))
	...
	for phase in ['train', 'dev', 'test']:
		score_df = pd.DataFrame(columns=['user_id', 'item_id', 'time', 'predictions'])
		if phase =='train':
			phase_type = 'dev'
		else:
			phase_type = phase
		dataset = model_name.Dataset(model, corpus, phase=phase_type)
		dataset.prepare()
		predictions = runner.predict(dataset)

		score_df['user_id'] = dataset.data['user_id']
		score_df['item_id'] = dataset.data['item_id']
		score_df['time'] =  dataset.data['time']
		score_df['predictions'] = predictions
		# import pdb; pdb.set_trace()

		score_df_dict[phase] = score_df
	
	score_df_all = pd.concat([score_df_dict[key] for key in ['train', 'dev', 'test']])
	model_name = '{0}{1}'.format(init_args.model_name,init_args.model_mode)
	# cur_time = datetime.datetime.now().strftime('%Y-%m-%d-%H:%M:%S')
	result_path = os.path.join(runner.log_path,runner.save_appendix, 'inference_scores-{}.csv'.format(model_name))
	utils.check_dir(result_path)
	score_df_all.to_csv(result_path, sep=args.sep, index=False)
...
def save_rec_results(dataset, runner, topk):
	...
	if init_args.model_mode == 'CTR': # CTR task 
		...
		users, pos_items, all_predictions = list(), list(), list()
		for i in range(len(dataset)):
			info = dataset[i]
			users.append(info['user_id'])
			pos_items.append(info['item_id'][0])
			all_predictions.append([pre for pre in predictions[i]])
		...
		# rec_df = pd.DataFrame(columns=['user_id', 'pos_item_id', 'rec_items', 'rec_predictions'])
		rec_df = pd.DataFrame(columns=['user_id', 'pos_item_id', 'all_predictions'])
		rec_df['user_id'] = users
		rec_df['pos_item_id'] = pos_items
		rec_df['all_predictions'] = all_predictions
