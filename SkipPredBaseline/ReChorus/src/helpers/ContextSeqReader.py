...
class ContextSeqReader(ContextReader):
	...
	def _append_his_info(self):
		...
		sort_df = pd.concat([data_dfs[phase][['user_id','item_id','time','phase','photo_id']+self.situation_feature_names] 
		...
		if self.seq_wo_photo:
			for idx, (uid, iid, t, pid) in enumerate(zip(sort_df['user_id'], sort_df['item_id'], sort_df['time'], sort_df['photo_id'])):
				if uid not in self.user_his:
					self.user_his[uid] = list()
				position.append(len(self.user_his[uid]))
				self.user_his[uid].append([iid, t, pid, situation_features[idx]])
		else:
