...
class DINBase(object):
	...
	def get_all_embedding(self, feed_dict, merge_all=True):
		...
		history_item_emb = torch.stack([self.embedding_dict[f](feed_dict['history_'+f].to(torch.long)) if f.endswith('_c') or f.endswith('_id')
...
class DINCTR(ContextSeqCTRModel, DINBase):
	...
	runner = 'CTRRunner'
	extra_log_args = ['emb_size','att_layers','add_historical_situations','history_max','dnn_layers']
