...
class CANBase(DIENBase):
	...
	def parse_model_args_can(parser):
		parser.add_argument('--induce_vec_size',type=int,default=128,
	...
	def forward(self, feed_dict):
		...
		# if item_ids.shape[-1]==2:
		# 	import pdb; pdb.set_trace()
		user_ids = feed_dict['user_id'] # B * item num
		history_item_ids = feed_dict['history_item_id'].long()
	...
	def gen_his_coation(self, induction, feed, mask):
		...
		for layer_idx in range(len(self.co_action_layers)):
			...
			output = (hidden_state.squeeze(3)*mask[:,:,None,None]).sum(dim=1)/mask.sum(dim=-1)[:,None,None] # B * item num * size
...
class CANTopK(ContextSeqModel, CANBase):
	...
	extra_log_args = ['emb_size','evolving_gru_type','fcn_hidden_layers','aux_hidden_layers','alpha_aux','induce_vec_size','orders']
