...
class GeneralModel(BaseModel):
	...
	class Dataset(BaseModel.Dataset):
		def _get_feed_dict(self, index):
			...
			elif 'inference' in self.corpus.dataset:
				neg_items = np.empty((0))
			else:
...
class SequentialModel(GeneralModel):
	...
	class Dataset(GeneralModel.Dataset):
		...
		def _get_feed_dict(self, index):
			...
			if self.corpus.seq_wo_photo:
				pid = self.data['photo_id_x'][index]
				x0, x1 = [], []
				count_lengths = 0
				if self.model.add_historical_situations and len(user_seq[0])>3:
					situation = {}
					for idx,c in enumerate(self.corpus.situation_feature_names):
						situation['history_'+c] = []
					for x in reversed(user_seq):
						if x[2]==pid:
							continue
						x0.append(x[0])
						x1.append(x[1])
						for idx,c in enumerate(self.corpus.situation_feature_names):
							situation['history_'+c].append(x[3][idx])
						count_lengths+=1
						if count_lengths==self.model.history_max:
							break
					for idx,c in enumerate(self.corpus.situation_feature_names):
						situation['history_'+c].reverse()
						feed_dict['history_'+c] = np.array(situation['history_'+c])
				else:
					for x in reversed(user_seq):
						if x[2]==pid:
							continue
				...
				x0.reverse()
				x1.reverse()
				feed_dict['history_items'] = np.array(x0)
				feed_dict['history_times'] = np.array(x1)
				feed_dict['lengths'] = len(feed_dict['history_items'])
			else:
				if self.model.history_max > 0:
					user_seq = user_seq[-self.model.history_max:]
				if hasattr(self.model, 'add_historical_situations'):
					if self.model.add_historical_situations and len(user_seq[0])>2:
						for idx,c in enumerate(self.corpus.situation_feature_names):
