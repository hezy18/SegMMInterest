...
class EvalDataLoader(AbstractDataLoader):
    ...
    def __init__(self, config, dataset, additional_dataset=None,
        ...
        self._get_eval_items_per_u(self.eval_u) # get all the items in the eval/test datasets
        # to device
    ...
    def get_eval_user_pos_items_dict(self):
        eval_users = self.eval_u.cpu()
        pos_items = self.eval_items_per_u
        eval_user_pos_items = {user.item(): pos_item for user, pos_item in zip(eval_users, pos_items)}
        return eval_user_pos_items
