...
class Trainer(AbstractTrainer):
   ...
   and some other features helpful for model training and evaluation.
    ...
    def __init__(self, config, model, mg=False):
        ...
        self.valid_metric = config['valid_metric'].lower() # TODO: hzy mark
    ...
    def _valid_epoch(self, valid_data, update_flag=False, is_test=False):
        ...
        Returns:
            ...
            dict: valid result
        """
        valid_result = self.evaluate(valid_data, is_test, update_flag)
        
        if self.valid_metric and self.valid_metric in valid_result:
            valid_score = valid_result[self.valid_metric] 
            print(f"{self.valid_metric} is not in valid_result")
        else:
            valid_score = valid_result['HR@5']
    ...
    def fit(self, train_data, valid_data=None, test_data=None, saved=False, verbose=True):
        ...
        # TODO: For debug:
        valid_score, valid_result = self._valid_epoch(valid_data, update_flag=True,is_test=True)
        for epoch_idx in range(self.start_epoch, self.epochs):
            ...
            if (epoch_idx + 1) % self.eval_step == 0:
                ...
                _, test_result = self._valid_epoch(test_data, update_flag, is_test=True) # TODO: hzy mark
                if verbose:
    ...
    @torch.no_grad()
    def evaluate(self, eval_data, is_test=False, update_flag=False, idx=0):
        r"""Evaluate the model based on the eval data.
        Returns:
        ...
        # if True: # TODO: hzy mark: save, is_test, update_flag
            # pos_items = eval_data.get_eval_items()
            # eval_users = eval_data.get_eval_users()
            # eval_user_pos_items = eval_data.get_eval_user_pos_items_dict()
            current_eval_users = batched_data[0].detach().cpu().numpy()
            # dataset_name = self.config['dataset']
            # model_name = self.config['model']
            # lr = self.learning_rate
            # score_list=[]
            for i, userID in enumerate(current_eval_users):
                scores_dict[userID] = scores[i, :]
            #     items = eval_user_pos_items[userID]
            #     for itemID in items:
                    # score = scores[i, itemID]
            #         score_list.append([userID,itemID,score])
            #         if userID not in score_dict:
            #             score_dict[userID]={}
            #         score_dict[userID][itemID]=score
            # dir_name = 'scoresleave'
            # if not os.path.exists(dir_name):
            #     os.makedirs(dir_name)
            # file_path = os.path.join(dir_name, '{}-{}-lr={}-{}.csv'.format(model_name, dataset_name, lr, get_local_time()))
            # score_df = pd.DataFrame(score_list, columns=['userID', 'itemID', 'score'])
            # score_df.to_csv(file_path, sep='\t', index=False)
        return self.evaluator.evaluate(batch_matrix_list, eval_data, scores_dict, logger = self.logger, is_test=is_test, idx=idx, update_flag=update_flag)
