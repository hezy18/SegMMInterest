...
class FREEDOM(GeneralRecommender):
    def __init__(self, config, dataset):
        ...
        self.new_pos_embedding = nn.Embedding(40, self.embedding_dim)
        nn.init.xavier_uniform_(self.user_embedding.weight)
        nn.init.xavier_uniform_(self.item_id_embedding.weight)

        self.learnable_param = nn.Parameter(torch.tensor(0.1))
        ...
        if os.path.exists(mm_adj_file):
            ...
            if self.v_feat is not None:
                indices, image_adj = self.get_knn_adj_mat_batch(self.image_embedding.weight.detach())
                self.mm_adj = image_adj
            if self.t_feat is not None:
                indices, text_adj = self.get_knn_adj_mat_batch(self.text_embedding.weight.detach())
    ...
    Here to add
    '''
    def compute_batch_knn(self, context_norm, start_idx, end_idx):
        batch_norm = context_norm[start_idx:end_idx]
        sim = torch.mm(batch_norm, batch_norm.transpose(0, 1))
        _, knn_ind = torch.topk(sim, self.knn_k, dim=-1)
        return knn_ind

    def get_knn_adj_mat_batch(self, mm_embeddings):
        context_norm = mm_embeddings.div(torch.norm(mm_embeddings, p=2, dim=-1, keepdim=True))
        total_size = context_norm.size(0)
        knn_inds_list = []
        for start_idx in range(0, total_size, 1024):
            end_idx = min(start_idx + self.batch_size, total_size)
            knn_ind = self.compute_batch_knn(context_norm, start_idx, end_idx)
            knn_inds_list.append(knn_ind)
        knn_inds = torch.cat(knn_inds_list, dim=0)
        indices0 = torch.arange(knn_ind.shape[0]).to(self.device)
        indices0 = torch.unsqueeze(indices0, 1)
        indices0 = indices0.expand(-1, self.knn_k)
        indices = torch.stack((torch.flatten(indices0), torch.flatten(knn_ind)), 0)
        adj_size = (total_size, total_size)
        return indices, self.compute_normalized_laplacian(indices, adj_size)
    ...
    def forward(self, adj):
        if self.v_feat is not None and self.v_feat.shape[-1]==1025:
            pos = self.v_feat[:,-1]*40
            pos = pos.to(torch.int)
            pos[pos<0]=0
            pos[pos>39]=39
            pos_embedding = self.new_pos_embedding(pos)
            h = self.item_id_embedding.weight + self.learnable_param * pos_embedding 
            # print('self.v_feat[:,-1]',self.v_feat[:,-1].min(),self.v_feat[:,-1].max())
            # print('pos_embedding',pos_embedding.min(),pos_embedding.max())
        else:
        ...
        if self.v_feat is not None and self.v_feat.shape[-1]==1025:
            ego_embeddings = torch.cat((self.user_embedding.weight, self.item_id_embedding.weight + self.learnable_param * pos_embedding), dim=0)
