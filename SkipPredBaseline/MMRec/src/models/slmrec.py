...
class SLMRec(GeneralRecommender):
    ...
    def compute(self):
        ...
        if self.v_feat is not None:
            self.v_dense_emb = self.v_dense(self.v_feat_norm)  # v=>id
            if self.v_feat.shape[-1]==1025:
                ...
                items_emb = items_emb +  self.learnable_param *  pos_embedding

        # if "Kwai" not in self.config["dataset"]:
    ...
    def feature_dropout(self, users_idx, items_idx): # FAC 不会到达这里
        users_emb = self.embedding_user.weight
        items_emb = self.embedding_item.weight
        ...
        # if "Kwai" not in self.config["data.input.dataset"]:
        if self.a_feat is not None:
        ...
        if "MM" not in self.config["data.input.dataset"]:
        ...
        if "MM" in self.config["data.input.dataset"]:
    ...
    def feature_masking(self, users_idx, items_idx, dropout=False): # FAC不会到达这里
        users_emb = self.embedding_user.weight
        items_emb = self.embedding_item.weight
    ...
    def fac(self, idx):
        ...
        if "MM" not in self.config["dataset"]:
    ...
    def getEmbedding(self, users, pos_items, neg_items):
        ...
        if self.v_feat is not None and self.v_feat.shape[-1]==1025:
            ...
            pos[pos>39]=39
            pos_embedding = self.new_pos_embedding(pos)
            pos_emb_ego = pos_emb_ego + self.learnable_param * pos_embedding[pos_items]
        ...
        if neg_items is None:
            ...
            if self.v_feat is not None and self.v_feat.shape[-1]==1025:
                ...
                pos[pos>39]=39
                pos_embedding = self.new_pos_embedding(pos)
                neg_emb_ego = neg_emb_ego + self.learnable_param * pos_embedding[pos_items]
    ...
    def create_u_embeding_i(self):
        ...
        self.new_pos_embedding = torch.nn.Embedding(40, self.latent_dim)
        ...
        mul_modal_cnt = 0
        if self.v_feat is not None:
            self.v_feat_norm = torch.nn.functional.normalize(self.v_feat, dim=1)
            self.v_dense = nn.Linear(self.v_feat_norm.shape[1], self.latent_dim)
