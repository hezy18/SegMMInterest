...
class BaseRunner(object):
	...
	def parse_runner_args(parser):
		...
		parser.add_argument('--auxillary_loss_weight', type=float, default=0,
							help='The weigth of auxillary loss') # TODO
		return parser
	...
	def evaluate_method(predictions: np.ndarray, topk: list, metrics: list) -> Dict[str, float]:
		...
		if (gt_rank!=1).sum()==0: # maybe all predictions are the same
	...
	def __init__(self, args):
		...
		self.auxillary_loss_weight = args.auxillary_loss_weight
	...
	def fit(self, dataset: BaseModel.Dataset, epoch=-1) -> float:
		...
		for batch in tqdm(dl, leave=False, desc='Epoch {:<3}'.format(epoch), ncols=100, mininterval=1):
			...
			if len(prediction.shape)==2: # only for ranking tasks
				...
				# import pdb; pdb.set_trace()
				restored_prediction[torch.arange(item_ids.shape[0]).unsqueeze(-1), indices] = prediction.to(torch.float32)
				out_dict['prediction'] = restored_prediction
			loss = model.loss(out_dict)
			try:
				if len(dataset.model.contrastive): #TODO
					loss += self.auxillary_loss_weight * out_dict['contrastive_loss']
			except:
				pass
			loss.backward()
