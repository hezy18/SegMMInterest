...
class DINBase(object):
	...
	def _define_params_DIN(self):
		...
		# change when loading item features in the code
		if len(self.clip_feature_path):
			self.item_frames_embedding = nn.Linear(1024, self.vec_size, bias=False )# reader 占有存储空间的策略
		...
		pre_size = (2*(self.item_feature_num+self.situation_feature_num)+self.item_feature_num
              +len(self.situation_context) + self.user_feature_num + self.item_frame_number) * self.vec_size
	...
	def get_all_embedding(self, feed_dict, merge_all=True):
		...
		item_feats_emb = torch.stack([self.embedding_dict[f](feed_dict[f]) if f.endswith('_c') or f.endswith('_id')
					...
					for f in self.item_context],dim=-2) # batch * item num * feature num * emb size
		...
		# change when loading item features in the code
		if len(self.clip_feature_path):
			item_frames_emb = self.item_frames_embedding(feed_dict['i_item_frames']).float() # batch * item num * clip num * emb size
			# situ_feats_emb = torch.cat((situ_feats_emb, item_frames_emb), dim=-2) # batch * item num * (feature+clip num) * emb size
			# 或者拼接在item feature里
			# 先尝试不进行拼接，加入all_context中
		...
		if merge_all:
			...
			if len(situ_feats_emb):
				all_context = torch.cat([item_feats_emb, user_feats_emb.unsqueeze(1).repeat(1,item_num,1,1),
							situ_feats_emb.unsqueeze(1).repeat(1,item_num,1,1)],dim=-2)
			else:
				all_context = torch.cat([item_feats_emb, user_feats_emb.unsqueeze(1).repeat(1,item_num,1,1),
							],dim=-2)
			if len(self.clip_feature_path):
				all_context = torch.cat([all_context,item_frames_emb], dim=-2)
			all_context_flatten = all_context.flatten(start_dim=-2)
			return history_emb, current_emb, all_context_flatten
		else: # 代码中未能处理这种情况
	...
	def forward(self, feed_dict):
		...
		'''
		current_emb.shape: torch.Size([256, 1, 2688]); torch.Size([256, 1, 128])
		history_emb.shape: torch.Size([256, 50, 128]); torch.Size([256, 50, 128])
		all_context.shape: torch.Size([256, 1, 2752]); torch.Size([256, 1, 2816])
		'''
