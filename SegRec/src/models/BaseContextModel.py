...
def get_context_feature(feed_dict, index, corpus, data):
	...
	for c in corpus.situation_feature_names+['time']:
...
def get_item_frames(feed_dict, id2item, useridframeid2lineid, clip_feat):
	item_ids, duration_list = feed_dict['item_id'], feed_dict['i_duration']
	i_item_frames = np.zeros((len(item_ids), 40, 1024), dtype=np.float32) # pad with zero
	for idx, (item_id, duration) in enumerate(zip(item_ids, duration_list)):
		clip_num = duration
		for clip_id in range(clip_num):
			key = f"{id2item[str(item_id)]}-{clip_id}"
			line_id = useridframeid2lineid[key]
			i_item_frames[0, clip_id, :] = clip_feat[line_id]
	feed_dict['i_item_frames'] = i_item_frames
	return feed_dict
	...
	class Dataset(GeneralModel.Dataset):
		...
		def _get_feed_dict(self, index):
			...
			if len(self.clip_feature_path):
				feed_dict = get_item_frames(feed_dict, self.id2item, self.useridframeid2lineid, self.clip_feat)
			return feed_dict
