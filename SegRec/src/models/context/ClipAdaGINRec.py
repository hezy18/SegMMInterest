...
class ClipAdaGINRecBase(ClipWDRecBase):
	@staticmethod
	def parse_model_args_ClipAdaGIN(parser):
	...
	def _define_init(self, args, corpus):
		...
		self._define_params_ClipAdaGIN()
		self.apply(self.init_weights)

	def _define_params_ClipAdaGIN(self):
	...
	def forward(self, feed_dict):
		...
		bsz, n_item, n_clip, _ = frame_concat_embed.shape
		user_embed = user_embed.unsqueeze(1).unsqueeze(1).repeat(1, n_item, n_clip,1) # batch_size * item_num * clip_num * embedding_dim
		...
		user_value = user_value.unsqueeze(1).unsqueeze(1).repeat(1, n_item, n_clip, 1) # batch_size * item_num * clip_num * embedding_dim
		linear_value = torch.cat([user_value, frame_concat_value], dim=-1) # batch_size * item_num * clip_num * (3*embedding_dim)
        
		FM_embedding = fm_vectors.reshape(bsz, n_item, n_clip, -1, self.vec_size)
		
		bsz, n_item, n_clip, n_feat, emb_dim = FM_embedding.shape
		
		# graph
		h_list = self.AutoGraph(FM_embedding) # bsz*n_item*n_clip, n_feat, emb_dim
		y_pred=0.
		for h, fsw in zip(h_list, self.final_score_weight):
			...
			if not self.only_use_last_layer:
				y_pred = y_pred * fsw
		clip_predictions = y_pred.view(bsz, n_item, n_clip)
		
		predictions = self._clip_integret_Rec(clip_predictions, feed_dict)
		return {'prediction':predictions}

class ClipAdaGINRecCTR(ContextCTRModel, ClipAdaGINRecBase):
	reader, runner = 'ContextReader', 'CTRRunner'
	extra_log_args = ['emb_size','fi_hidden_units','w_hidden_units','num_gnn_layers','loss_n','adjust_interest_weight','clip_weight_path','auxillary_loss_weight']
	
	@staticmethod
	def parse_model_args(parser):
		parser = ClipAdaGINRecBase.parse_model_args_ClipAdaGIN(parser)
	...
	def forward(self, feed_dict):
		out_dict = ClipAdaGINRecBase.forward(self, feed_dict)
		out_dict['prediction'] = out_dict['prediction'].view(-1).sigmoid()
		out_dict['label'] = feed_dict['label'].view(-1)
		return out_dict	

class ClipAdaGINRecRanking(ContextModel,ClipAdaGINRecBase):
	...
	def forward(self, feed_dict):
		return ClipAdaGINRecBase.forward(self, feed_dict)
...
class AutoGraph_Layer(nn.Module):
	...
	def forward(self, feature_emb):
		bsz, n_item, n_clip, n_feat, emb_dim = feature_emb.shape
		feature_emb = feature_emb.view(bsz*n_item*n_clip, n_feat, emb_dim)
		h = feature_emb
		h_list = []
