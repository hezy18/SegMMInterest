...
def load_data_default():
    all_df = pd.read_csv('data/KuaiRand_leave.inter', sep='\t')
    ...
    defautl_id  = all_df['frame_id'].max()+2
    ...
    print('if default', all_df['item_id'].isin([default_id]).any())
    ...
    df_unique['item_id'] = default_id
    test_df = pd.concat([all_df, df_unique], ignore_index=True)

    RAW_PATH_SAVE = os.path.join('./', 'KuaiRandinferenceDefault')
