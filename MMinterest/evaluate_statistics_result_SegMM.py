...
from utils import BaseReaderSeq_SegMM, FrameDatasetSeq_SegMM, DataCollator, DataLoader, LargeHDF5Cache
...
def statistics_dataset(reader, useridframeid2lineid,feat_memmap, args):
    reader.data_df['train_dev'] = pd.concat([reader.data_df[key] for key in ['train', 'dev']])
    if args.eval_cold=='sampleData':
        train_dev_dataset = FrameDatasetSeq_SegMM_sampled(corpus=reader, lineid_map=useridframeid2lineid, feat_memmap=feat_memmap, phase='train', shuffle=False, do_scale_image_to_01=True, image_resize=True, verbose=False)
    else:
        train_dev_dataset = FrameDatasetSeq_SegMM(corpus=reader, lineid_map=useridframeid2lineid, feat_memmap=feat_memmap, phase='train_dev', shuffle=False, do_scale_image_to_01=True, image_resize=True, verbose=False)
    ...
    prob_view_all = float(num_view_all / num_duration_all) # number
...
def main(args, test_dataloader, statis_results, test_type):
    ...
    for local_valid_step, test_batch in tqdm.tqdm(enumerate(test_dataloader)):
        ...
        if len(args.eval_cold):
            ...
            # if args.debug and local_valid_step==0: import pdb; pdb.set_trace()
    ...
    # with open(f"eval_results_new/SegMM/results_all_points/{test_type}__{args.TOP_K_permutation}_{args.TOP_K_mask}.json", "w") as fw:
...
if __name__=="__main__":
    ...
    parser.add_argument('--eval_cold', type=str, default='sampleData', choices=['','test', 'sampleData'])
    ...
    if args.statistics:
        statis_results = statistics_dataset(reader, useridframeid2lineid,feat_memmap, args)
        torch.save(statis_results, f'statis_results_SegMM_1219.pth')
    else:
        statis_results={}
    if args.eval_cold=='sampleData':
    ...
    if len(args.eval_cold):
        with open(f'eval_results_new/SegMM/results_all_points/statistic_final_0114_eval_cold_{args.TOP_K_permutation}_{args.TOP_K_mask}_{seed_value}.json',"w") as fw:
        ...
        df.to_csv(f'eval_results_new/SegMM/results_all_points/statistic_final_0114_eval_cold_{args.TOP_K_permutation}_{args.TOP_K_mask}_{seed_value}.csv',index=False)

        with open(f'eval_results_new/SegMM/results_all_points/statistic_final_0114_eval_hot_{args.TOP_K_permutation}_{args.TOP_K_mask}_{seed_value}.json',"w") as fw:
        ...
        for key, values in final_hot.items():
            ...
            # df = df.append(row, ignore_index=True)
        df = pd.concat([pd.DataFrame([row], columns=['key']+all_keys) for row in rows], ignore_index=True)
        df.to_csv(f'eval_results_new/SegMM/results_all_points/statistic_final_0114_eval_hot_{args.TOP_K_permutation}_{args.TOP_K_mask}_{seed_value}.csv',index=False)

    with open(f'eval_results_new/SegMM/results_all_points/statistic_final_0114_mask_{args.TOP_K_permutation}_{args.TOP_K_mask}_{seed_value}.json',"w") as fw:
        json.dump(final, fw)
    ...
    for key, values in final.items():
        ...
        # df = df.append(row, ignore_index=True)
    df = pd.concat([pd.DataFrame([row], columns=['key']+all_keys) for row in rows], ignore_index=True)
    df.to_csv(f'eval_results_new/SegMM/results_all_points/statistic_final_0114_mask_{args.TOP_K_permutation}_{args.TOP_K_mask}_{seed_value}.csv',index=False)