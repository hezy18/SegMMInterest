...
def main(args):
    ...
    for epoch in range(args.epochs):
        ...
        for local_step, batch in enumerate(train_dataloader):
            ...
            if args.ablation_type=='noUser':
                user_id_rand = torch.randint(1, reader.n_users, batch["user_id"].shape, device=batch["user_id"].device)
                train_output = model(usr_image = user_id_rand, usr_id = user_id_rand, usr_mask = batch["user_mask"], 
    ...
    if args.test_model:
        ...
        with open(f"eval_results_KuaiRand/results_all_points/{param_dir}_{args.TOP_K_permutation}_{args.TOP_K_mask}_{args.mask_loss}_{args.use_pe}_{args.fusion_heads}_{cur_time}.json", "w") as fw:
            json.dump(results_list, fw)
        with open(f"eval_results_KuaiRand/results_all_points/{param_dir}_{args.TOP_K_permutation}_{args.TOP_K_mask}_{args.mask_loss}_{args.use_pe}_{args.fusion_heads}_final_{cur_time}.json", "w") as fw:
    ...
    plt.savefig(f"pics/KuaiRand/train_loss_{cur_time}_{args.num_layers_enc}_{args.exposure_prob_type}_{args.learning_rate}_{args.weight_decay}_{args.learnable_bias}_{args.loss_type}_{args.loss_weight}_{args.input_type}_{args.mask_loss}_{args.use_pe}_{args.fusion_heads}_earlystop_focal.png")
    plt.figure(figsize=(10,8))
    plt.plot(range(len(total_valid_loss_metrics['valid_loss'])), total_valid_loss_metrics['valid_loss'])
    plt.title("Valid Loss")
    plt.savefig(f"pics/KuaiRand/valid_loss_{cur_time}_{args.num_layers_enc}_{args.exposure_prob_type}_{args.learning_rate}_{args.weight_decay}_{args.learnable_bias}_{args.loss_type}_{args.loss_weight}_{args.input_type}_{args.mask_loss}_{args.use_pe}_{args.fusion_heads}_earlystop_focal.png")
...
if __name__ == "__main__":
    ...
    parser.add_argument('--eval_type_list', type=str, default='JaccardSim,ProbAUC,LeaveMSE,LeaveCTR,LeaveCTR_view,TOP_K', help='combination of JaccardSim, ProbAUC, LeaveMSE,LeaveCTR, LeaveCTR_view, TOP_K split by ,')
    ...
    parser.add_argument('--ablation_type', type=str, default='ours', choices=['ours','CrossAtt','SelfAtt','noPos','noUser','CrossMLP','SelfMLP','noUser_SelfAtt','w/oAtt'])
