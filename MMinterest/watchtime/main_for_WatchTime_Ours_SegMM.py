...
def prepareMemmap():
    mapping_path = "data/SegMM_photoidframeid2lineid.json"
    useridframeid2lineid = json.load(open(mapping_path))
    total_line = len(useridframeid2lineid)
    feat_memmap = np.memmap("data/SegMM_feat_memmap.dat", dtype='float32', mode='r', shape=(total_line, 1024))
...
def compute_final_result(results_list,sample_count=None):
    ...
    if 'pred_leave' in results_list:
        ...
        mse = mean_squared_error(view_lengths, pred_view_lengths)
        mae = mean_absolute_error(view_lengths, pred_view_lengths)
        final_result_dict['pred_leave'][index] = (mse, mae)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--input_type', type=dict, default={'user':'both','photo':'both'})
    ...
    parser.add_argument('--record_train_detail', type=int, default=0) # TODO:add later
