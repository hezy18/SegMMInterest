...
def get_threshold_of_playtime(df, perc=0.6):
    if 'play_time_ms_x' in df.columns:
        q = (df['play_time_ms_x']/5000).quantile(perc)
    else:
        q = (df['playing_time_x']/5000).quantile(perc)
    return q
...
def valid_model(args, valid_dataloader, model,criterion, q_threshold):
    model.eval()
    tmp_valid_loss = []
    for local_valid_step, valid_batch in enumerate(valid_dataloader):
        ...
        ctr_label = (valid_batch['play_time']>q_threshold).float()
        loss = criterion(output.squeeze(), ctr_label.squeeze())
...
def main(args):
    ...
    q_threshold = get_threshold_of_playtime(reader.data_df['train'],0.6)
    ...
    for epoch in range(args.epochs):
        for local_step, batch in enumerate(train_dataloader):
            ...
            ctr_label = (batch['play_time']>q_threshold).float()
            loss = criterion(output.squeeze(), ctr_label.squeeze())
            ...
            if (local_step+1)%args.valid_step == 0: # evaluation_step
                valid_loss = valid_model(args, valid_dataloader, model, criterion, q_threshold)
    ...
    if args.test_model:
        ...
        for local_test_step, test_batch in tqdm.tqdm(enumerate(test_dataloader)):
            ...
            expected_watch_time = output.squeeze() * test_batch['duration'].squeeze()
            
            watchTime_label = test_batch['play_time'] 
            watchTime_label[watchTime_label>40]=40
            vr_label = watchTime_label.squeeze() / 40
            
            labels_all.append(watchTime_label.int().squeeze().cpu().numpy())
            preds_all.append(torch.round(expected_watch_time).squeeze().cpu().numpy())
...
if __name__ == "__main__":
    ...
    parser.add_argument('--weight_decay', type=float, default=1e-4) # 目前代码中并没有用
